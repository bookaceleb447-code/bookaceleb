...
export const CelebrityRegisterPage = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create additional profile record
      const slug = formData.name.toLowerCase().trim().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
      const ab = Math.random().toString(36).substring(2, 4);
      await setDoc(doc(db, 'celebrityProfiles', uid), {
        celebId: uid,
        celebName: formData.name,
        slug,
        isLocked: true,
        referralLink: `${window.location.origin}/ref/${ab}/${slug}`,
        bookingPrice: 0,
        fanCardPrice: 0,
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#020617] relative overflow-hidden">
      ...
      >
        <div className="text-center mb-10">
          <h1 className="text-4xl font-display font-bold mb-3 tracking-tighter uppercase underline decoration-primary/30 text-white italic">Celebrity Onboarding</h1>
          <p className="text-white/40">Secure your spot in the global elite circle.</p>
        </div>

        <form onSubmit={handleRegister} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            <label className="block text-xs font-bold uppercase tracking-widest mb-2 opacity-60 text-white/60">
              Public Name / Stage Name
            </label>
            <input 
              type="text" 
              value={formData.name}
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest mb-2 opacity-60 text-white/60">Professional Email</label>
            <input 
              type="email" 
              value={formData.email}
              onChange={(e) => setFormData({...formData, email: e.target.value})}
              className="w-full bg-black/40 border border-white/10 rounded-2xl px-5 py-4 focus:outline-none focus:border-primary/50 transition-all font-medium text-white"
              placeholder="management@idol.com"
              required
            />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            >
              {loading ? 'Initializing Protocol...' : 'Register as Celebrity'}
            </button>
          </div>
        </form>
        ...
        <div className="mt-8 text-center text-sans">
          <p className="text-sm text-white/40 font-medium">
            Already registered? <Link to="/admin/login" className="text-white hover:underline font-bold">Admin Sign In</Link>
          </p>
        </div>
      </motion.div>
    </div>
