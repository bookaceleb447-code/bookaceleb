                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2 space-y-8">
                        <form onSubmit={handleSubmit} className="glass rounded-[3rem] p-4 sm:p-8 md:p-14 space-y-10 border border-white/5">
                            <h2 className="text-3xl font-display font-bold uppercase tracking-tighter italic block border-b border-white/5 pb-4">Schedule Your Moment</h2>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <label className="block text-[10px] uppercase font-black opacity-30 mb-2 tracking-widest italic text-white/40">Event Type Protocol</label>
                                    <select 
                                ...
                                <div className="space-y-2">
                                    <label className="block text-[10px] uppercase font-black opacity-30 mb-2 tracking-widest italic text-white/40">Temporal Window</label>
                                    <input 
                                      type="datetime-local" 
                            ...
                            <div>
                                <label className="block text-[10px] uppercase font-black opacity-30 mb-2 tracking-widest italic flex justify-between text-white/40">
                                    Chronos Allocation (Hours) <span>${celeb?.bookingPrice}/hr</span>
                                </label>
                                <input 
                            ...
                            <div className="space-y-6">
                                <h3 className="font-bold uppercase tracking-widest text-white/40 border-l-4 border-primary pl-4 italic">Identification Matrix</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <input placeholder="Full Name Signature" value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} className="bg-black/40 border border-white/10 rounded-2xl p-4 font-medium text-white outline-none focus:border-primary/50" required />
                                    <input type="email" placeholder="Communication Email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="bg-black/40 border border-white/10 rounded-2xl p-4 font-medium text-white outline-none focus:border-primary/50" required />
                                    <input placeholder="Contact Cipher (Phone)" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="bg-black/40 border border-white/10 rounded-2xl p-4 font-medium text-white outline-none focus:border-primary/50" required />
                                    <input placeholder="Destination Geometry (Location)" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="bg-black/40 border border-white/10 rounded-2xl p-4 font-medium text-white outline-none focus:border-primary/50" required />
                                </div>
                            </div>

                            {/* Payment Method Protocol Selection */}
                            <div className="space-y-6 pt-6 border-t border-white/5">
                                <h3 className="font-bold uppercase tracking-widest text-white/40 border-l-4 border-primary pl-4 italic">Payment Gateway Selection</h3>
                                ...
                                <div className="p-4 sm:p-6 bg-black/40 border border-white/10 rounded-2xl space-y-4">
                                    ...
                                    {paymentMethod === 'crypto' && (
                                        <div className="space-y-4 text-sm text-left">
                                            <p className="text-[10px] uppercase font-black tracking-widest text-primary">Celebrity Crypto Wallet</p>
                                    ...
                                    {paymentMethod === 'giftcard' && (
                                        <div className="space-y-3 text-sm text-left">
                                            <p className="text-[10px] uppercase font-black tracking-widest text-primary">Celebrity Gift Card Option</p>
                            ...
                            <div>
                                <h4 className="text-[10px] uppercase font-black opacity-30 mb-4 tracking-widest italic text-white/40">Payment Proof Screenshot</h4>
                                <div className="h-48 rounded-[2rem] bg-black/40 border-2 border-dashed border-white/10 flex flex-col items-center justify-center text-center px-10 relative cursor-pointer hover:border-primary/50 transition-all overflow-hidden group">
                                     {file ? <div className="text-primary font-bold animate-pulse">{file.name}</div> : (
                                       <>
                                         ...
                                         <p className="text-[10px] font-black uppercase tracking-widest opacity-30 text-white">Upload Payment Proof Receipt</p>
                                       </>
                                     )}
                    ...
                    <div className="space-y-8">
                        <div className="glass rounded-[2rem] p-4 sm:p-8 bg-slate-900 text-white relative overflow-hidden group border border-white/5 shadow-2xl">
                           ...
                           <h4 className="text-[10px] font-black uppercase italic tracking-widest opacity-40 mb-6">Total Price</h4>
                           ...
                           <div className="p-6 bg-white/5 rounded-2xl border border-white/10 space-y-4 text-left">
                               <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Payment Gateway</p>
                               <div className="flex justify-between items-center">
                                   <p className="font-bold italic uppercase tracking-wide text-xs">
                                       {paymentMethod === 'bank' ? 'Direct Bank Transfer' : paymentMethod === 'crypto' ? 'Crypto Gateway' : 'Gift Card/Voucher'}
                                   </p>
                        ...
                        <div className="glass rounded-[2rem] p-8 space-y-6 border border-white/5">
                            <h4 className="font-bold uppercase tracking-widest italic text-xs text-white/40">Secure Payments</h4>
                            <div className="flex items-center gap-4 p-4 bg-primary/5 rounded-2xl border border-primary/10">
                                <ShieldCheck size={20} className="text-primary" />
                                <p className="text-[10px] font-black uppercase tracking-widest opacity-60 leading-tight">Secure Booking. Your payment is safely held until after the booking completes.</p>
                            </div>
                        </div>
                    </div>
                </div>
